...
double	depth)
{
	...
	dataBuf[0] = depth;
	if ((file_id=H5Fopen(fileName, H5F_ACC_RDWR, H5P_DEFAULT))<=0) { fprintf(stderr,"after file open, file_id = %lld\n",file_id); ERROR_PATH(file_id) }
...
hid_t dataType)
{
	...
	sz = H5Tget_size(dataType);
	printf("		data type = %lld,  size of one element = %ld bytes\n",dataType,sz);

	order = H5Tget_order(dataType);
    if (order == H5T_ORDER_LE) printf("		Little endian order\n");
    else if (order == H5T_ORDER_BE) printf("		Big endian order\n");
	else printf("		Unknown order for data type %lld,  may be string\n",dataType);
